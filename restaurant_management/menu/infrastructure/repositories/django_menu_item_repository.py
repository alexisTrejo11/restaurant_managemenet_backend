...
class DjangoMenuItemRepository(MenuItemRepository):
      ...
      def get_all(self) -> List[MenuItem]:
         ...
         if menu_items_cache and len(menu_items_cache) > 0:
            return menu_items_cache

         menu_items = MenuItemModel.objects.all().order_by('id')
         if menu_items:
            return [MenuItemMapper.model_to_domain(model) for model in menu_items]
         return []
      ...
      def _create(self, menu_item: MenuItem) -> MenuItem:
         new_item_model = MenuItemMapper.domain_to_model(menu_item)
         
         new_item_model.save()

         menu_item_created = MenuItemMapper.model_to_domain(new_item_model)

         new_cache_key = self.cache_manager.get_cache_key(menu_item_created.id)         
         self.cache_manager.set(new_cache_key, menu_item_created)
      ...
      def _get_by_id_db(self, item_id: int, raise_expection=False) -> Optional[MenuItem]:
         try:   
            ...
            return MenuItemMapper.model_to_domain(model)
      ...
      def _get_query_set_from_cache(self) -> List[MenuItem]:
         ...
         if menu_items_cache and len(menu_items_cache) > 0:
            return [MenuItemMapper.model_to_domain(model) for model in menu_items_cache]
